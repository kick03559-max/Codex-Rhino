...
class RhinoMCPServer:
        ...
        try:
            ...
            while self.running:
                ...
                try:
                    ...
                    # Rhino document APIs must run on Rhino's main thread.
                    # InvokeOnUiThread is reliable in Rhino 7 where the Idle
                    # event may not fire for a script-loaded IronPython
                    # handler.  The network thread waits for the UI callback,
                    # then sends the response back over the socket.
                    response = self._execute_on_rhino_thread(command)
    ...
    def _execute_on_rhino_thread(self, command):
        """Execute one command on Rhino's UI thread and return its response."""
        result_holder = []
        completed = threading.Event()

        def run_command():
            try:
                result_holder.append(self.execute_command(command))
            except Exception as e:
                log_message("Error executing command: {0}".format(str(e)))
                traceback.print_exc()
                result_holder.append({"status": "error", "message": str(e)})
            finally:
                completed.set()

        Rhino.RhinoApp.InvokeOnUiThread(Action(run_command))
        if not completed.wait(120.0):
            raise Exception("Timed out waiting for Rhino UI thread")

        if result_holder:
            return result_holder[0]
        return {"status": "error", "message": "Rhino UI callback returned no response"}

    def _send_response(self, client, response):
        """Send a JSON response, allowing large viewport payloads."""
        response_json = json.dumps(response)
        chunk_size = 14485760  # 10MB chunks
        response_bytes = response_json.encode('utf-8')
        for i in range(0, len(response_bytes), chunk_size):
            client.sendall(response_bytes[i:i + chunk_size])
    ...
    def _get_rhino_layers(self):
        ...
        try:
            ...
            for layer in doc.Layers:
                # Rhino 7's Layer class does not expose ObjectCount.
                # Count document objects by layer index instead.
                object_count = 0
    ...
    def _get_rhino_selected_objects(self, params):
        """Get objects currently selected in Rhino without opening a picker."""
        try:
            include_lights = params.get("include_lights", False)
            include_grips = params.get("include_grips", False)
            selected_objects = []

            # Rhino 7 exposes the current selection through ObjectTable.  The
            # previous implementation opened GetObject, which was interactive
            # and also referenced Rhino 8-only properties.
            try:
                objects = sc.doc.Objects.GetSelectedObjects(include_lights, include_grips)
            except Exception:
                objects = [obj for obj in sc.doc.Objects if obj.IsSelected]
